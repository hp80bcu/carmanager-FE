...
export default function Nav() {
  ...
  const handleMyCarClick = (event: React.MouseEvent<HTMLAnchorElement>) => {
    event.preventDefault(); // 페이지 이동 방지
    if (userId) {
      navigate(`/mycar`, { state: { userId, username } });
    } else {
      // userId가 존재하지 않는 경우
      alert('로그인 후 이용해주세요.');
    }
    ...
    <>
      ...
      <div className="h-container">
        ...
        <div className="item right">
          <Link to={"/mycar"} className="custom-link" state={{username}} onClick={handleMyCarClick}>
        ...
        {userId !== null && (
              <div className="item right">
                <Link to={`/users/${userId}`} state={{ userId }} className="custom-link">
